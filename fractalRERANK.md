...
graph TD
    ...
    PreprocessDocuments["Preprocess Documents"]
    SplitIntoChunks["Split Into Chunks"]
    TrainAndSaveModel["Train & Save Model"]
    LoadModel["Load Model"]
    SentenceVector["Sentence Vector"]
    SemanticSearch["Semantic Search Fractal Chunking"]
    RecursiveChunking["Recursive Chunking"]
    FinalResults["Final Results"]

    %% Connections
    Start --> ReadDocument
    ReadDocument --> PreprocessDocuments
    PreprocessDocuments --> SplitIntoChunks
    SplitIntoChunks --> TrainAndSaveModel
    TrainAndSaveModel --> LoadModel
    LoadModel --> SentenceVector
    SentenceVector --> SemanticSearch
    SemanticSearch --> RecursiveChunking
    RecursiveChunking --> SplitIntoChunks

    %% Recursive Connections to Emphasize Feedback Loops
    RecursiveChunking -->|Recursive Split & Refine| RecursiveChunking
    TrainAndSaveModel -->|Feedback: Adjust Model| TrainAndSaveModel
    LoadModel -->|Feedback: Reload & Refine| LoadModel
    SentenceVector -->|Iterative Refinement| SentenceVector
    SemanticSearch -->|Update Search Criteria| SemanticSearch

    %% Additional Feedback Loops
    SplitIntoChunks -->|Back to Splitting| SplitIntoChunks
    FinalResults -->|Review and Reset| Start
    Start -->|Reinitialize with Adjustments| Start

    %% Annotations for Clarity
    Start -.->|Initialize Process Flow| Start
    SplitIntoChunks -.->|Dynamically Adjust Based on Content & Query| SplitIntoChunks
    SemanticSearch -.->|Refine Search with Each Iteration| SemanticSearch
    LoadModel -.->|Adjust Based on Feedback| LoadModel
```
